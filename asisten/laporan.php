...
// ------ LOGIKA UNTUK SEMUA PROSES (CREATE, UPDATE, DELETE) ------
$message = '';
$message_type = '';

// Proses UPDATE (Memberi Nilai)
...
// Proses DELETE (Menghapus Laporan)
if (isset($_GET['delete'])) {
    $laporan_id_to_delete = $_GET['delete'];
    
    // 1. Ambil nama file laporan dari database sebelum dihapus
    $stmt_get_file = $conn->prepare("SELECT file_laporan FROM laporan WHERE id = ?");
    $stmt_get_file->bind_param("i", $laporan_id_to_delete);
    $stmt_get_file->execute();
    $file_to_delete_result = $stmt_get_file->get_result();
    ...
    if ($stmt_delete->execute()) {
        // 3. Jika record berhasil dihapus, hapus juga file fisiknya dari server
        if ($file_to_delete && !empty($file_to_delete['file_laporan'])) {
            $file_path = '../uploads/' . $file_to_delete['file_laporan'];
        ...
        <tbody class="text-gray-700">
            ...
            <tr class="border-b hover:bg-gray-50">
                ...
                <td class="py-3 px-4 flex items-center gap-4">
                    <a href="laporan.php?nilai=<?php echo $row['id']; ?>" class="text-blue-600 hover:text-blue-800 font-semibold">
                        <?php echo ($row['status'] == 'Dinilai') ? 'Edit Nilai' : 'Beri Nilai'; ?>
                    </a>
                    <!-- TOMBOL HAPUS BARU -->
                    <a href="laporan.php?delete=<?php echo $row['id']; ?>" class="text-red-600 hover:text-red-800 font-semibold" onclick="return confirm('Anda yakin ingin menghapus laporan ini secara permanen?');">
                        Hapus
                    </a>
                </td>
            </tr>
