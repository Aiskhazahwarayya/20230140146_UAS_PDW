...
// ------ LOGIKA UNTUK PROSES UPLOAD LAPORAN (CREATE/UPDATE) ------
...
// ------ LOGIKA BARU UNTUK MENGHAPUS LAPORAN (DELETE) ------
if (isset($_GET['delete_laporan'])) {
    $laporan_id_to_delete = $_GET['delete_laporan'];
    
    // Ambil nama file dan pastikan laporan ini milik mahasiswa yang login
    $stmt_get = $conn->prepare("SELECT file_laporan FROM laporan WHERE id = ? AND mahasiswa_id = ? AND status = 'Terkumpul'");
    $stmt_get->bind_param("ii", $laporan_id_to_delete, $mahasiswa_id);
    $stmt_get->execute();
    $result_get = $stmt_get->get_result();

    if ($result_get->num_rows > 0) {
        $file_to_delete = $result_get->fetch_assoc();
        
        // Hapus record dari database
        $stmt_delete = $conn->prepare("DELETE FROM laporan WHERE id = ?");
        $stmt_delete->bind_param("i", $laporan_id_to_delete);
        if ($stmt_delete->execute()) {
            // Hapus file fisik dari server
            if (!empty($file_to_delete['file_laporan']) && file_exists('../uploads/' . $file_to_delete['file_laporan'])) {
                unlink('../uploads/' . $file_to_delete['file_laporan']);
            }
        ...
        $message = "Laporan tidak dapat dihapus (mungkin sudah dinilai atau bukan milik Anda).";
        $message_type = 'error';
    }
    $stmt_get->close();
}
            ...
            <?php if ($laporan): ?>
                <div class="mt-2 p-3 rounded-lg bg-gray-50">
                    <div class="flex justify-between items-start">
                        ...
                        <!-- TOMBOL HAPUS BARU (Hanya muncul jika belum dinilai) -->
                        <?php if ($laporan['status'] == 'Terkumpul'): ?>
                        <div>
                            <a href="course_detail.php?id=<?php echo $praktikum_id; ?>&delete_laporan=<?php echo $laporan['id']; ?>" class="text-red-500 hover:text-red-700 text-sm" onclick="return confirm('Anda yakin ingin menghapus laporan ini? Anda bisa mengunggahnya kembali setelah ini.');">
                                Hapus Laporan
                            </a>
                        </div>
                        <?php endif; ?>
                    ...
                    <div class="border-t mt-3 pt-3">
                        ...
                        <p><strong>Feedback Asisten:</strong> <?php echo !empty($laporan['feedback']) ? nl2br(htmlspecialchars($laporan['feedback'])) : '<i>Tidak ada feedback.</i>'; ?></p>
                    </div>
                    <?php endif; ?>
                </div>
            <?php else: ?>
