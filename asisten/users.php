...
<?php if ($message): ?>
<div class="mb-4 px-4 py-3 rounded-lg <?php echo ($message_type == 'success') ? 'bg-green-100 border-green-400 text-green-700' : 'bg-red-100 border-red-400 text-red-700'; ?>">
    ...
    <form action="users.php" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="nama" id="nama" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="<?php echo htmlspecialchars($user_to_edit['nama'] ?? ''); ?>" required>
            </div>
            <div>
                <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                <input type="email" name="email" id="email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="<?php echo htmlspecialchars($user_to_edit['email'] ?? ''); ?>" required>
            </div>
            <div>
                <label for="password" class="block text-sm font-medium text-gray-700">Password</label>
                <input type="password" name="password" id="password" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" <?php echo isset($user_to_edit) ? '' : 'required'; ?>>
        ...
        <div class="mt-6 text-right">
            <button type="submit" name="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Simpan</button>
        </div>
    </form>
</div>
        ...
        <tbody class="text-gray-700">
            ...
            <tr class="border-b hover:bg-gray-50">
                ...
                <td class="py-3 px-4 whitespace-nowrap">
                    <a href="users.php?edit=<?php echo $row['id']; ?>" class="text-blue-600 hover:text-blue-800 font-semibold mr-4">Edit</a>
                    <?php if ($row['id'] != ($_SESSION['user_id'] ?? 0)): // Sembunyikan tombol hapus untuk diri sendiri ?>
                    <a href="users.php?delete=<?php echo $row['id']; ?>" class="text-red-600 hover:text-red-800 font-semibold" onclick="return confirm('Anda yakin ingin menghapus pengguna ini?');">Hapus</a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endwhile; ?>
